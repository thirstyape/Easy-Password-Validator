...
namespace Easy_Password_Validator
{
    ...
    /// </summary>
    public class PatternMapService
    {
        /// <summary>
        /// Configures a new pattern mapping service instance
        /// </summary>
        /// <param name="useKeyboardMap">Specifies whether to load the default keyboard map</param>
        public PatternMapService(bool useKeyboardMap = true)
        {
            Map = useKeyboardMap ? GetKeyboardMap() : new List<PatternMapItem>();
        }

        /// <summary>
        /// The map to use when detecting patterns
        /// </summary>
        public List<PatternMapItem> Map { get; set; }
        ...
        public bool IsNeighbor(char key, char neighbor)
        {
            return Map.Any(x => 
        ...
        public PatternMapItem GetMapItem(char key)
        {
        ...
        public IEnumerable<string> GetPatterns(string test)
        {
            return test.Select((c, i) => test.Substring(i)
                .TakeWhile(x => IsNeighbor(c, x))
                .ToString()
            );
        }
        ...
        /// </summary>
        /// <param name="test">The string to check</param>
        public bool IsPattern(string test)
        {
            var result = true;
            ...
            {
                if (last != default(char) && IsNeighbor(c, last) == false)
                    return false;
        ...
        /// </summary>
        private List<PatternMapItem> GetKeyboardMap()
        {
            return new List<PatternMapItem>()
            {
